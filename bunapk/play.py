...
from bunapk.auth import build_headers
from bunapk.protobuf import ProtoDecoder, extract_strings
...
def _proto_headers(auth: dict, languages: Optional[list[str]] = None) -> dict:
    headers = build_headers(auth, languages=languages)
...
def get_delivery(package: str, version_code: int, auth: dict, languages: Optional[list[str]] = None) -> DeliveryResult:
    """Fetch download URLs for base APK, splits, and OBB files."""
    headers = _proto_headers(auth, languages=languages)
